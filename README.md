...
- [ ] Take a look at the [ArticleControllerTest](src/test/kotlin/org/approvej/workshop/service/adapters/demanding/http/article/ArticleControllerTest.kt).
  ...
  The test should fail and open a diff view of the new received file `ArticleControllerTest-get articles-received.txt` and the new and empty approved file `ArticleControllerTest-get articles-approved.txt` right next to the test file.
...
- [ ] Re-run the test.
  ...
  It will fail again because the `articleNumber` values are randomized by the test builder.
  This is a first taste of a concept we'll explore in depth later: [Scrubbing](https://approvej.org/#scrubbing).
  For now, add a scrubber to replace the dynamic article numbers with a placeholder:
  ...
  Re-approve the received file and re-run the test.
...
- [ ] Apply the `JsonPrintFormat` to the result using the `printedAs` method.
  ...
  This time the diff view opens for two new files: `ArticleControllerTest-get articles-received.json` and `ArticleControllerTest-get articles-approved.json`.
...
You already got a first taste of scrubbing when dealing with the randomized article numbers.
Now let's explore scrubbing in more depth with the shopping cart tests, which have more dynamic data.
Parts of the data may be generated by the code: IDs of any kind, timestamps, dates etc.

To deal with such problems, ApproveJ comes with a feature called [Scrubbing](https://approvej.org/#scrubbing).

- [ ] Take a look at the [ShoppingCartControllerTest](src/test/kotlin/org/approvej/workshop/service/adapters/demanding/http/shoppingcart/ShoppingCartControllerTest.kt).
...
- [ ] The received file again contains the result object in a single line, which makes it hard to overlook differences.
  ...
  ApproveJ offers a variety of PrintFormats to turn POJOs into readable strings.
...
I hope this workshop was beneficial for you and the concept of Approval Testing in general and ApproveJ are clear to you, and you will be able to apply it in your daily work.
...
But so far, we only looked into the demanding adapter code, even though contracts exist on the providing side as well.

I plan to extend the feature set of ApproveJ into that direction.
